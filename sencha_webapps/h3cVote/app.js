...
function enableWxConfig(){   
    ...
    var urlStr = "/svtbase/MpService?req=getJsapiSignature&appId="+appId+"&url="+ curUrl;
...
function checkIsVoted(openId){
    var isVoteTmp = "error";
    
                        Ext.Ajax.request({
                            url: '/svtbase/HzServices',
                            method: 'GET',
                            async: false,
                            params: { req: 'checkIsVoted', appId:appId, voteFromOpenId: openId},
                            success: function ( result, request) { 
                                ...
                                if( jsonObj.status == "success" && jsonObj.isVote == "true" ) {
                                    ...
                                    isVoteTmp = "true";
                                    voteToProject = jsonObj.voteToProject
...
function showInfo(msg){
    ...
    var html = '<img src="/media/h3cVote/'+'index'+'.jpg"  width="'+window.innerWidth+'"  />'
    item.setHtml(html);
    /*
...
Ext.application({
    ...
    if( (timestampClient -timestampSerInt ) > 45*1000 ){
        ...
        //window.location.href='https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxba6e25f5a7acbf6d&redirect_uri=http%3A%2F%2Fhz.byodwork.cn%2Fsvtbase%2FMpService&response_type=code&scope=snsapi_base&state=wxba6e25f5a7acbf6dEndVote#wechat_redirect';
        MsgBoxExit("", '请关注"华三深圳办"公众号后进行投票');
        return;     
    }
    else{
    ...
    enableWxConfig();
    isVote = checkIsVoted(openId);
    console.log("Check is voted:"+isVote);
    ...
    CandStore =  Ext.create('Ext.data.Store', {
        ...
        */
        ...
        proxy: {
            ...
            url: '/svtbase/HzServices?req=getCandList&appId='+appId,
            limitParam: 'limit',
            pageParam: 'page',
            reader: {
                type: "json",
      ...
      CandStoreByPoll =  Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url: '/svtbase/HzServices?req=getCandListBySortByPoll&appId='+appId,
            limitParam: 'limit',
            pageParam: 'page',
            reader: {
                type: "json",
        ...
        {  
            ...
            onItemDisclosure: {
                ...
                handler: function (record, btn, index) {
                    ...
                    //var str = '<img src="/media/'+index+'.jpg"  width="'+ScreenWidth+'"  />';
                    
                    if( isVote == "true"){
                        MsgBox("", "您已投给 "+voteToProject+",感谢参与!");
                        return;
                    }
                    
                    var ObjVote = new Object();
                    ObjVote.voteToCandidateId = record.data.candidateId;
                    ObjVote.voteFromOpenId = openId;
                    ObjVote.appId = appId;
                    
                    voteToProject = record.data.project;
                    ...
                    function voteCommit(){
                        ...
                        Ext.Ajax.request({
                            url: '/svtbase/HzServices?req=saveVoteInfo',
                            method: 'POST',
                            params: { req: 'saveVoteInfo', voteInfo: jsonVoteInfo},
                            success: function ( result, request) {
                                var jsonObj = JSON.parse(result.responseText);
                    ...
                    VoteConfirmBox(voteCommit, record.data.project);
                  
                },
            },
            
            itemTpl: new Ext.XTemplate(
                '<p>{[this.outputHtml(values.candidateId)]}</p>',     
                {               
                    outputHtml:  function(valueIn)
                    {           
                        var index1 = CandStore.find('candidateId', valueIn);
                        ...
                        */
                        ...
                        + '当前票数 &nbsp;<font size=4 color=#04BD00 >'+myrecord.get('poll') + '</font></p><p style = "margin:4px 5px 4px 5px">'
                        +'</p></font></div></td></tr></table>'
                        +'<p style = "margin:10px 5px 4px 5px" align=center > <font size=4>'+ myrecord.get('project')+ '</font></p>'
                        +'<p style = "margin:15px 5px 8px 5px; text-align:justify;" ><font size=3> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+ myrecord.get('detail') + '</font></p>';
        ...
        items: [
             {
                ...
                html: '<img src="/media/h3cVote/'+'index'+'.jpg"  width="'+ScreenWidth+'"  />'
             },
...
　　　　　　　　loadingText: '正在刷新……',
            ...
            itemTpl: new Ext.XTemplate(
                ...
                {               
                    outputHtml:  function(valueIn)
                    {   
                        var index1 = CandStoreByPoll.findExact('candidateId', valueIn);
